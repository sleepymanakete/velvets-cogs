...
Install (once you have the cog folder added as a repo or loaded locally):
    [p]load inactivitykick
...
class InactivityKick(commands.Cog):
    ...
    async def on_member_join(self, member: discord.Member):
        # Start the clock the moment they join, so new members aren't kicked instantly
        ...
        for guild in self.bot.guilds:
            ...
            for member, last_seen in inactive:
                try:
                    ...
                    pass  # DMs closed, skip notification
                try:
                    await guild.kick(member, reason=f"Inactive for {settings['days']}+ days")
                except discord.Forbidden:
                    pass  # missing permission, skip silently (or log if you add logging)
    ...
    async def setdays(self, ctx: commands.Context, days: int):
        ...
        await ctx.send(embed=make_embed(
            "Threshold updated",
            f"Members will now be kicked after **{days} days** of inactivity.",
            COLOR_OK,
        ))
    ...
    async def toggle(self, ctx: commands.Context):
        ...
        await ctx.send(embed=make_embed(
            "Auto-kick toggled",
            f"Auto-kicking for inactivity is now **{state}**.",
            COLOR_OK if not current else COLOR_WARN,
        ))
    ...
    async def exemptrole(self, ctx: commands.Context, role: discord.Role):
        ...
        await ctx.send(embed=make_embed(
            "Role exempted",
            f"**{role.name}** is now exempt from inactivity kicks.",
            COLOR_OK,
        ))
